...
function ReactionAnimator() {
  ...
  useEffect(() => {
    if (uid) void logPractice(uid, "reactions");
  }, [uid]);
  ...
  useEffect(() => {
    ...
    const draw = () => {
      ...
      if (raw < 0.2) {
        setStepTitle("Bonds break");
        setStepText(STEP_BREAK);
      } else if (raw < 0.8) {
        setStepTitle("Atoms rearrange");
        setStepText(STEP_REARRANGE);
      } else {
        setStepTitle("Products form");
        setStepText(reaction.formStep);
      }
      ...
      const cur = R.atoms.map((a, i) => {
        ...
        return [from[0] + (to[0] - from[0]) * travel, from[1] + (to[1] - from[1]) * travel] as [
          number,
          number,
        ];
      });
      ...
      for (const cl of R.clusters)
        ctx.fillText(cl.formula, px(cl.centre[0]), py(cl.centre[1]) - rAtom * 2.6);
      ...
      if (breakP < 1) {
        ...
        ctx.lineWidth = rAtom * 0.5;
        for (const [a, b] of R.bonds) {
          ctx.beginPath();
          ctx.moveTo(px(cur[a][0]), py(cur[a][1]));
          ctx.lineTo(px(cur[b][0]), py(cur[b][1]));
          ctx.stroke();
        }
      }
      if (formP > 0) {
        ...
        for (const [a, b] of P.bonds) {
          const ra = invMatch[a],
            rb = invMatch[b];
          ctx.beginPath();
          ctx.moveTo(px(cur[ra][0]), py(cur[ra][1]));
          ctx.lineTo(px(cur[rb][0]), py(cur[rb][1]));
          ctx.stroke();
        }
      }
      ctx.globalAlpha = 1;
      ...
      for (let i = 0; i < R.atoms.length; i++) {
        ...
        ctx.fillStyle = glow;
        ctx.beginPath();
        ctx.arc(x, y, rAtom * 1.6, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = col;
        ctx.beginPath();
        ctx.arc(x, y, rAtom, 0, Math.PI * 2);
        ctx.fill();
        ctx.fillStyle = "rgba(0,0,0,0.78)";
        ctx.font = `700 ${rAtom * 1.05}px "EB Garamond", serif`;
        ctx.textAlign = "center";
        ctx.textBaseline = "middle";
        ctx.fillText(R.atoms[i].el, x, y);
      }
      ...
      ctx.lineWidth = 2;
      ctx.beginPath();
      ctx.moveTo(w * 0.47, h / 2);
      ctx.lineTo(w * 0.53, h / 2);
      ctx.stroke();
      ctx.beginPath();
      ctx.moveTo(w * 0.53, h / 2);
      ctx.lineTo(w * 0.515, h / 2 - 6);
      ctx.lineTo(w * 0.515, h / 2 + 6);
      ctx.closePath();
  ...
  return (
    <StudentShell title="Reactions">
      ...
      <div className="mb-5 space-y-3">
        {TYPE_ORDER.map((t) => (
          <div key={t}>
            <div className="mb-1.5 text-[10px] uppercase tracking-[0.2em] text-parchment/45">
              {t}
            </div>
      ...
      {/* Stage */}
      <div
        className="atom-stage-panel relative overflow-hidden rounded-2xl"
        style={{
          border: `1px solid color-mix(in oklab, ${ACCENT} 30%, transparent)`,
          boxShadow: `0 0 60px -26px ${ACCENT}`,
        }}
      >
        {/* side headers */}
        <span className="pointer-events-none absolute left-4 top-3 text-[10px] uppercase tracking-[0.25em] text-parchment/45">
          Reactants
        </span>
        <span className="pointer-events-none absolute right-4 top-3 text-[10px] uppercase tracking-[0.25em] text-teal/55">
          Products
        </span>
      ...
      <div className="mt-4 flex flex-wrap items-center gap-3">
        ...
        <button onClick={restart} className="btn-ghost-arcane text-sm">
      ...
      >
        ...
        <p className="mb-3 text-xs text-parchment/60">
          The same atoms are on both sides of the arrow — they've only swapped partners.
        </p>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  border: `1px solid color-mix(in oklab, ${col} 32%, transparent)`,
                }}
              >
                <span
                  className="flex h-6 w-6 items-center justify-center rounded-md text-xs font-semibold"
                  style={{ background: col, color: "#0b1220" }}
                >
                  {el}
                </span>
        ...
        <p className="mt-3 text-sm leading-relaxed text-parchment/70">
          ...
          <span className="text-gold">conservation of mass</span>: the reason chemical equations
          must balance.
        </p>
      </div>
      ...
      <div className="mt-4">
        <ConceptCard title={`This is a ${reaction.type.toLowerCase()} reaction`}>
          {TYPE_INFO[reaction.type]}
        </ConceptCard>
      </div>
    </StudentShell>
  );
